...
async function fetchWithRateLimitInfo(
  url,
  connection,
  retries = 3,
  backoff = 1000
  ...
  try {
    ...
    const rateLimitInfo = {
      "X-Rate-Limit-Time-Window-Ms":
        response.headers["x-rate-limit-time-window-ms"],
      "X-Rate-Limit-Time-Reset-Ms":
        response.headers["x-rate-limit-time-reset-ms"],
      "X-Rate-Limit-Requests-Quota":
        response.headers["x-rate-limit-requests-quota"],
      "X-Rate-Limit-Requests-Left":
        response.headers["x-rate-limit-requests-left"],
    };
...
async function checkCustomFieldFeed(config, productId) {
  ...
  try {
    ...
    if (error.code === "EADDRNOTAVAIL") {
      console.error(
        `Error de dirección no disponible al buscar campos personalizados para el producto: ${productId}. Reintentando...`
      );
...
async function manageProductProcessingFeed(config,feedId,totalPages,currentUrl) {
  ...
  let checkpoint = await getCheckpoint(storeHash, currentUrl);
  let currentPage = checkpoint.currentPage || 1;
  let totalValidCount = checkpoint.totalValidCount || 0;

  if (checkpoint.isCompleted) {
    console.log(`URL ya fue completada. Saltando...`);
    return totalValidCount;
  }

  const segmentSize = Math.ceil(totalPages / divisionOfPages);
  console.log(
    `Iniciando desde la página ${currentPage} \nPara la URL: ${currentUrl}`
  );
  ...
  for (let i = 0; i < divisionOfPages; i++) {
    ...
    const transformedProductos = await Promise.all(
      validProductIds.map(async (product) => {
        try {
          ...
          console.error(
            `Error al transformar el producto: ${product.id}, Error: ${error.message}`
          );
    ...
    const filteredProducts = transformedProductos.filter(
    ...
    console.log(
      `Checkpoint creado en ${currentPage} \nPara la URL ${currentUrl}`
    );
  }

  // Resetear el checkpoint y marcar como completado si hemos llegado al final
  if (currentPage >= totalPages) {
    ...
    console.log(
      `Checkpoint reseteado a 1 y marcado como completado para store_hash ${storeHash} \nURL ${currentUrl}`
    );
  }
...
async function countPagesFeed(config) {
  ...
  try {
    ...
    if (
      ...
      !initialResponse.meta.pagination
    ) {
...
async function countPagesForDisabledAndZeroPrice(config) {
  ...
  try {
    ...
    const fetchProductSKUs = async (filter) => {
      const baseUrl = `https://api.bigcommerce.com/stores/${storeHash}/v3/catalog/products`;
      const optionsGET = await getConfig(config);
          ...
          promises.push(
            fetchWithRetry(pageUrl, optionsGET)
              ...
              .catch((error) => {
                ...
                // Continuar con las siguientes páginas a pesar del error
              })
          );

          // Log cada 50 páginas
          if ((page - startPage + 1) % 50 === 0) {
            console.log(`Processed up to page: ${page}`);
          }
        }
      ...
      // Procesar las páginas en lotes
      for (let i = 1; i <= totalPages; i += batchSize) {
        const startPage = i;
        const endPage = Math.min(i + batchSize - 1, totalPages); // Asegura que no exceda el total de páginas
    ...
    // Contar y obtener SKUs con diferentes filtros
    const disabledSKUs = await fetchProductSKUs("availability=disabled");
    const zeroPriceSKUs = await fetchProductSKUs("price=0");
    const notVisibleSKUs = await fetchProductSKUs("is_visible=false");

    // Combinar los SKUs en un Set para eliminar duplicados
    const uniqueSKUs = Array.from(
      new Set([...disabledSKUs, ...zeroPriceSKUs, ...notVisibleSKUs])
    );
...
async function manageProductSync(config, totalPages, googleMerchantSKUs) {
  ...
  for (let i = 0; i < divisionOfPages; i++) {
    const endPage =
      currentPage + segmentSize - 1 > totalPages
        ? totalPages
        : currentPage + segmentSize - 1;
    const products = await getAvailableProductsFeed(
      config,
      currentPage,
      endPage
    );
  ...
  const skusToDelete = googleMerchantSKUs.filter(
    (sku) => !allBigCommerceSKUs.includes(sku)
  );

  if (skusToDelete.length > 0) {
    await deleteBatchProducts(skusToDelete, config); // Batch delete products from Google Merchant
    console.log(
      `Deleted ${skusToDelete.length} products from Google Merchant not present in BigCommerce.`
    );
  }

  console.log(
    `Processed total of ${allBigCommerceSKUs.length} products from BigCommerce.`
  );
  console.log(
    `Total valid products in manageProductProcessing: ${totalValidCount}`
  ); // Display total valid products processed
...
async function findMissingProductsInBigCommerce(
  config,
  totalPages,
  googleMerchantSKUs
) {
  ...
  for (let i = 0; i < divisionOfPages; i++) {
    ...
    const bigCommerceSKUs = products.allValidProductIds.map(
      (product) => product.sku
    );
    allBigCommerceSKUs = allBigCommerceSKUs.concat(bigCommerceSKUs);
    currentPage = endPage + 1;
  ...
  console.log(
    `Total SKUs to delete from Google Merchant: ${skusToDelete.length}`
  );
...
async function manageSingleProductProcessing(config, productId) {
  ...
  try {
    ...
    console.log(
      `Producto ${productId} procesado y guardado en Google Merchant`
    );
  } catch (error) {
