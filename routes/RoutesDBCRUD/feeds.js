...
routerFeeds.post("/feeds/createFeed", authenticateToken, async (req, res) => {
  ...
  const intervalUnit = feedData.recurrence
    ? parseInt(feedData.recurrence.intervalUnit, 10)
    : undefined;
  const selectedDaysArray = feedData.recurrence
    ? feedData.recurrence.selectedDays
    : undefined;
  const selectedDays = selectedDaysArray
    ? selectedDaysArray.join(";")
    : undefined; // Convertir la lista en un string

  console.log("Datos: ----------------", intervalUnit, selectedDays);

  feedData.selectedDays = selectedDays;
  feedData.intervalHour = intervalUnit;
  feedData.isActive = feedData.recurrence
    ? Boolean(feedData.recurrence.isActive)
    : false;
...
routerFeeds.put(
  ...
  async (req, res) => {
    ...
    const intervalUnit = updateData.recurrence
      ? parseInt(updateData.recurrence.intervalUnit, 10)
      : undefined;
    const selectedDaysArray = updateData.recurrence
      ? updateData.recurrence.selectedDays
      : undefined;
    const selectedDays = selectedDaysArray
      ? selectedDaysArray.join(";")
      : undefined; // Convertir la lista en un string
    ...
    updateData.isActive = updateData.recurrence
      ? Boolean(updateData.recurrence.isActive)
      : false;
      ...
      if (feed) {
        ...
        setImmediate(async () => {
          try {
            ...
            const conteoByTipo = await manageProductProcessing(
              config,
              conteoPages
            );
            ...
            const [totalProductsGM, totalProductsBC, preorderProducts] =
              await Promise.all([
            ...
            res.status(200).json({
              message: "Sincronización completada y feed actualizado",
            });
          } catch (error) {
            console.error(
            ...
            res.status(404).json({
              message:
                "Hubo un error en la sincronización, verifique los datos ingresados",
            });
          }
        });
      } else {
        ...
        setImmediate(async () => {
          try {
            for (const urlActual of url.url) {
              ...
              console.log("Conteo: ", conteoPages);
              const conteoByTipo = await manageProductProcessingFeed(
                config,
                feedId,
                conteoPages,
                urlActual
    ...
    if (feed) {
      ...
      setImmediate(async () => {
        try {
          ...
          const conteoByTipo = await manageProductSync(
            config,
            conteoPages,
            productsSKUs
          );
          ...
          res
            .status(200)
            .json({ message: "Sincronización completada y feed actualizado" });
        } catch (error) {
          console.error(
            "Error durante la sincronización en segundo plano:",
            error
    ...
    try {
      ...
      try {
        ...
        if (!googleCredentialsValid) {
          return res
            .status(400)
            .json({ message: "Credenciales de Google Merchant inválidas" });
        }
      } catch (error) {
        console.error("Error al verificar las credenciales de Google:");
        return res.status(400).json({
          message: "Error al verificar las credenciales de Google Merchant",
        });
      }

      try {
        bigCommerceCredentialsValid = await verifyBigCommerceCredentials(
          config
        );
        if (!bigCommerceCredentialsValid) {
          return res
            .status(400)
            .json({ message: "Credenciales de BigCommerce inválidas" });
        }
      } catch (error) {
        console.error("Error al verificar las credenciales de BigCommerce:");
        return res.status(400).json({
          message: "Error al verificar las credenciales de BigCommerce",
        });
      }
      ...
      if (googleCredentialsValid && bigCommerceCredentialsValid) {
        ...
        await updateTable(
          "feeds",
          { isActive: configCron.isActive },
          "feed_id",
          feedId
        );
        res.status(200).json({
          message: `Cron job ${
            configCron.isActive ? "activado" : "desactivado"
          }`,
        });
      } else {
        res.status(400).json({
          message:
            "Error en las credenciales, no se puede cambiar el estado del cron job",
        });
      }
    } catch (error) {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (feed) {
        ...
        console.log(
          `Iniciando sincronización para el producto ${productId} del feed ${feedId}`
        );

        // Ejecutar la sincronización del producto en segundo plano
        setImmediate(async () => {
          try {
            await manageSingleProductProcessing(config, productId);
            res.status(200).json({
              message: "Producto sincronizado exitosamente con Google Merchant",
            });
          } catch (error) {
            console.error(
              "Error durante la sincronización del producto:",
              error
            );
            res.status(500).json({
              message: "Hubo un error durante la sincronización del producto",
            });
          }
        });
      } else {
